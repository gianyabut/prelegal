import { NDAFormData, MNDATermOption, ConfidentialityTermOption } from '@/types/nda';

// Full standard terms from Mutual-NDA.md (Common Paper MNDA v1.0)
...
export function getMndaTermLabel(term: MNDATermOption): string {
  if (term === 'indefinite') return 'Until terminated';
  return `${term} year${term !== '1' ? 's' : ''}`;
}

export function getConfidentialityTermLabel(term: ConfidentialityTermOption): string {
  if (term === 'perpetual') return 'In perpetuity';
  return `${term} year${term !== '1' ? 's' : ''}`;
}

export function getMndaTermText(term: MNDATermOption): string {
  if (term === 'indefinite') return 'until terminated in accordance with the terms of the MNDA';
  return `${term} year${term !== '1' ? 's' : ''} from Effective Date`;
}

export function getConfidentialityTermText(term: ConfidentialityTermOption): string {
  if (term === 'perpetual') return 'in perpetuity';
  return `${term} year${term !== '1' ? 's' : ''} from Effective Date, but in the case of trade secrets until Confidential Information is no longer considered a trade secret under applicable laws`;
}
...
export function resolveStandardTerms(data: NDAFormData): string {
  const purpose = data.purpose || '[Purpose]';
  const effectiveDate = data.effectiveDate ? formatDisplayDate(data.effectiveDate) : '[Effective Date]';
  const mndaTerm = getMndaTermText(data.mndaTerm);
  const confidentialityTerm = getConfidentialityTermText(data.termOfConfidentiality);
